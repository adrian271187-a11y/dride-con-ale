...
export default function AdminLayout() {
  ...
  return (
    <div style={{ display:'flex',minHeight:'100vh' }}>
      <aside style={{ width:210,background:'#0A2A1E',display:'flex',flexDirection:'column',padding:'20px 12px',gap:4,flexShrink:0 }}>
        ...
        {nav.map(({ to, label }) => (
          <NavLink key={to} to={to} end={to==='/admin'}
            style={({ isActive }) => ({ color:isActive?'#9FE1CB':'rgba(255,255,255,0.55)',background:isActive?'rgba(29,158,117,0.2)':'transparent',textDecoration:'none',fontSize:13,padding:'8px 10px',borderRadius:8,fontWeight:isActive?600:400 })}>
            {label}
          </NavLink>
        ))}
      ...
      <main style={{ flex:1,background:'#F8F8F6',overflowY:'auto' }}><Outlet /></main>
    </div>
  )
}
