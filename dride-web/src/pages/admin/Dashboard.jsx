  ...
  return (
    <div style={{padding:28}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:28}}>
        {[{label:'Reservas',value:stats.reservas,icon:'🎫'},{label:'Clientes',value:stats.clientes,icon:'👥'},{label:'Paquetes activos',value:stats.paquetes,icon:'🧳'},{label:'Ingresos',value:`$${stats.ingresos.toLocaleString()}`,icon:'💰'}].map(({label,value,icon})=>(
      ...
      <div style={{background:'#fff',border:'1px solid #E5E5E3',borderRadius:12,padding:20}}>
        ...
        {recientes.length===0 ? <p style={{color:'#888'}}>Sin reservas aún.</p> :
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
            <thead><tr style={{borderBottom:'1px solid #E5E5E3'}}>
              {['Código','Total','Estado'].map(h=><th key={h} style={{padding:'8px 10px',textAlign:'left',color:'#888',fontWeight:600,fontSize:11}}>{h}</th>)}
            </tr></thead>
